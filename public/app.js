...
let currentFolder = null; // null = root of category
let isEditMode = false;
let allDocsCache = []; // Cache for filtering
...
function loadDocuments() {
    ...
    // Fetch ALL docs for user (Realtime or One-time)
...
function renderView() {
    ...
    if (currentFolder) {
        ...
        // 2. ROOT CATEGORY VIEW (Show Folders + Loose Files)
        backBtn.style.display = 'none';
        folderStatus.style.display = 'none';

        // Find unique folders in this category
        ...
        // Render Loose Files (Files with NO folder)
...
function createFileCard(data) {
    ...
    div.innerHTML = `
        <div class="card-actions">
            <button class="action-btn btn-edit" onclick="openEditModal('${data.id}', '${data.title}', '${data.category}', '${data.folder || ''}')">
                <i class="fas fa-pen"></i>
            </button>
            <button class="action-btn btn-delete" onclick="deleteDocument('${data.id}')">
        ...
        <a href="${data.url}" target="_blank" style="text-decoration:none;">
            ...
            <div class="card-info">
                ...
                <div class="card-meta">
                    <span>${data.date}</span>
                    <span style="font-size:0.7rem; background:#eee; padding:2px 6px; border-radius:4px;">${data.category}</span>
                </div>
            </div>
        </a>
    `;
...
function saveDocument() {
    ...
    const folder = document.getElementById('docFolder').value.trim(); // New Folder Field
...
function openModal(mode) {
    if (mode === 'add') {
        ...
        document.getElementById('docFolder').value = currentFolder || ''; // Pre-fill folder if inside one
...
// Update Datalist for folder suggestions based on selected category
function updateFolderList() {
    const cat = document.getElementById('docCategory').value;
    const list = document.getElementById('folderSuggestions');
    list.innerHTML = '';
    
    // Find unique folders existing in this category
    const existingFolders = [...new Set(
        allDocsCache.filter(d => d.category === cat).map(d => d.folder).filter(f => f)
    )];
